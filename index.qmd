...
Instances of [E61 Time Primitive] and [E52 Time-Span] are better expressed as RDF literals of type `xsd:date`, `xsd:time`, `xsd:dateTime`, `xsd:dateTimeStamp`, or `xsd:gDay`, `xsd:gMonthDay`, `xsd:gMonth`, `xsd:gYear`, or `xsd:gYearMonth` if applicable. More complex values, in particular explicit intervals can be expressed with [EDTF] and [Time Ontology]\ (both mapped to each other with [EDTF in RDF]). Support of complex temporal data is limited in SPARQL, so there is no consensus yet.^[See discussion to extend SPARQL 1.2 [for simple dates](https://github.com/w3c/sparql-dev/blob/main/SEP/SEP-0002/sep-0002.md).]

~~~{.ttl #lst-temporal-primitives}
...
<TitanticSinking> a crm:E81_Transformation ;
  ...
  crm:P4_has_time-span 
    ...
    "1912-04-15"^^edtf:EDTF .                       # imprecise date with EDTF
~~~

@Doerr2020 recommend to use additional properties for temporal intervals ([E52 Time-Span]):
...
The use of these properties may lead to false assumption of precision and it introduced a solitary solution to a problem also addressed outside of CIDOC. For this reasons these additional properties should *not* be used in favour of EDTF and/or Time Ontology, for instance like this: 

~~~{.ttl}
<WW1> a crm:E52_Time-Span , time:TemporalEntity .
...
Temporal CRM properties such as [P86 falls within] should also be replaced by corresponding properties from Time Ontology such as [time:intervalIn].
...
See also a more detailled description of combining CRM and Time Ontology by @Nys2018.
...
CRM class **[E32 Authority Document]** and CRM property **[P71 lists]** MUST NOT be used in RDF but corresponding SKOS RDF classes [ConceptScheme] and [inScheme] instead. Applications MAY define [ConceptScheme] as subclass of [E31 Document] and [inScheme] as subproperty of [P67 refers to].
