...
Instances of [E61 Time Primitive] and [E52 Time-Span] SHOULD be expressed as RDF literals with one of the following data types or as instance of [TemporalEntity] or its subclasses from [Time Ontology].
:
...
- `xsd:gYear`
- `xsd:gYearMonth`
- `edtf:EDTF` for more complex values, such as intervals or uncertain dates, in [EDTF] syntax
  - `edtf:EDTF-level0`
  - `edtf:EDTF-level1`
  - `edtf:EDTF-level2`
...
Support of simple temporal data is included in most SPARQL processors although it is not part of the SPARQL 1.2 specification (see [proposal SEP-0002](https://github.com/w3c/sparql-dev/blob/main/SEP/SEP-0002/sep-0002.md)).

More complex temoral data can be expressed with the [Time Ontology] as discussed by @Nys2018. EDTF can be mapped to Time Ontology via [EDTF in RDF]
...
Temporal CRM properties such as [P86 falls within] should also be replaced by corresponding properties from Time Ontology such as [intervalIn].
...
~~~ttl
dcterms:Agent   owl:equivalentClass crm:E39_Actor .
...
If structured data is not available, bibliographic references can also be expressed with plain strings.

<!-- TODO: example
Entities (authors, publishers...) SHOULD be referenced by established URI (e.g. ORCID, GND...) if possible.

~~~ttl
_:WWWProposal a crm:
<https://orcid.org/0000-0003-1279-3709> a foaf:Person .
